...
- `Description` `(string: <optional>)` - A free form a human-readable, useful
  description of the ACL Role. It must not exceed 256 characters.

- `Policies` `(array<ACLRolePolicyLink>)` The list of policies that should be
  applied to the role. An ACLRolePolicyLink is an object with a "Name" field to
  specify a policy.
